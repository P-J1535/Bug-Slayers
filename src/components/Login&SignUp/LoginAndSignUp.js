...
function LoginAndSignUp() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('https://rfc2rnvg-4000.inc1.devtunnels.ms/user/signUp', {
        method: 'POST',
        headers: {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('https://rfc2rnvg-4000.inc1.devtunnels.ms/user/login', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
