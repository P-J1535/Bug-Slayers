    ...
    const countOfCpp = () =>{
       const count   = questionData.filter(res=> res.subject_name === 'C++').length ;
       return count
    }

    const countofjava  =()=> {
        const count   = questionData.filter(res=> res.subject_name === 'Java').length ;
       return count
    }
    const countofReactjs =() =>{
        const count   = questionData.filter(res=> res.subject_name === 'ReactJS').length ;
       return count
    }
...
<Box sx={{display:'flex',justifyContent:'flex-end',my:1}}>
                ...
                <Table>
                    <TableHead>
                        <TableRow className={classes.tableHead}>
                            ...
                            <TableCell  sx={{minWidth:'300px' ,maxWidth:'300px'}}>Options</TableCell>
  ...
  {Array.isArray(question.option)
    ...
    : question.option}
</TableCell>                                <TableCell>{question.Answer}</TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
